...
from models.model import create_model_diffu, Att_Diffuse_model
from trainer import model_train, LSHT_inference
from collections import Counter

import numpy as np

# 兼容旧写法 np.int / np.float / np.bool
...
parser.add_argument('--device', type=str, default=None, help='GPU device automatically selected')
parser.add_argument('--num_gpu', type=int, default=None, help='Auto-detect number of GPUs')
...
parser.add_argument('--model', type=str, default='freqdiffusion',
                    choices=['freqdiffusion', 'diffurec', 'sasrec'],
...
if args.num_gpu > 1:
    ...
    print(f"✅ Detected {args.num_gpu} GPUs, using DataParallel.")
...
def create_model(args):
    if args.model == 'freqdiffusion':
        from models.model import create_model_diffu, Att_Diffuse_model
        diffu = create_model_diffu(args)
        model = Att_Diffuse_model(diffu, args)
        print("✅ Initialized model: FreqDiffusion")

    elif args.model == 'diffurec':
        from models.diffurec import DiffuRec
        from models.model import Att_Diffuse_model
        diffu = DiffuRec(args)
        model = Att_Diffuse_model(diffu, args)
        print("✅ Initialized model: DiffuRec")

    elif args.model == 'sasrec':
        from models.sasrec import SASRec
        model = SASRec(args)
        print("✅ Initialized model: SASRec")
